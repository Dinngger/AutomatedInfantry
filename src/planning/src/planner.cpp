...
//  ros::Time t0 = ros::Time::now();
  ...
  std::vector<std::vector<bool>> binMap(width);

  for (int x = 0; x < width; x++) { binMap[x].resize(height); }
...
void Planner::plan() {
  ...
  if (validStart && validGoal) {
    ...
    std::vector<Node2D> nodes2D = std::vector<Node2D>(width * height);
    ...
    Node2D n2DGoal(x, y, 0, 0, nullptr);
    // __________
    // DEBUG GOAL
    ...
    // set theta to a value (0,2PI]
    t = Helper::normalizeHeadingRad(t);
    Node2D n2DStart(x, y, 0, 0, nullptr);

    // ____________________
    // 先进行二维空间中的路径规划
    // 由于nodes2D存储池同样用于后续2D A*启发式函数的获取，所有对此存储池的操作都应为从终点到起点
    Node2D* n2DSolution = Algorithm::aStar(n2DGoal, n2DStart, nodes2D, width, height, configurationSpace, visualization);
    std::vector<Node2D> path2D;
    Algorithm::tracePath2D(n2DSolution, 0, path2D);
    std::reverse(path2D.begin(), path2D.end());
    std::cout << "2D path size: " << path2D.size() << std::endl;
    rawPath.clear();
    rawPath.updatePath2D(path2D);
    rawPath.publishPath();
